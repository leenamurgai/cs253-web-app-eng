...
def query2():
    filteredlist = filter(pick_sub_id,links)
    filteredlist.sort(key=lambda x:x.submitted_time)
    return sortedlist
...
def query7():
    ...
    filteredlist = cursor.fetchall()
    filteredlistIDs=[]
    for x in filteredlist:
        filteredlistIDs.append(x[0])
        print x[0]
...
def build_link_index1():
    return dict([(l.id,l) for l in links])

def build_link_index2():
    index = {}
    for l in links:
        index[l.id]=l
    return index

def link_by_id1(link_id):
    for l in links:
        if l.id == link_id:
            return l

link_index = build_link_index1()
def link_by_id(link_id):
    return link_index.get(link_id)#[link_id]
